...
- **R1 — Audit chunk_id formats; fix misleading example in system-prompt.** Findings: the chunker emits 12 distinct prefixes by design (`pokemon:`, `mega:`, `move:`, `item:`, `ability:`, `mega-ability:`, `updated-attack:`, `team:`, `usage:`, `matchup:`, `txt:`, `md:`) and the citation validator IS strict (`Set.has(id)` exact match). The "heterogeneity" observed across runs was just legitimate multi-source IDs, not chaos. **Real bug found:** [src/lib/system-prompt.ts](../src/lib/system-prompt.ts) line 123 example listed `knowledge:meta_snapshot.md#top-cores` as a chunk_id format — that prefix and `#anchor` syntax DO NOT exist anywhere in the chunker. Models seeing this example may have been hallucinating IDs in that fake format. Replaced with the full real-shape list + a correct `md:data/knowledge/meta_snapshot.md:5` example. Also added "never invent ID formats" instruction. Prompt version bumped from `v4.1-claims-json-tightened` to `v4.2-real-chunkid-examples-noprefix`.
- **R2 — Enrich `pokedex` tool output with Pikalytics competitive context.** Two new helpers in [src/lib/tools.ts](../src/lib/tools.ts): `loadPikalyticsContext()` reads `pikalytics_usage.csv` once at first call and caches the parse (89 rows); `lookupPikalytics(name)` returns `{rank, usagePct, topMoves, topItems, topAbilities, topTeammates}`. Pokedex case appends a `competitive` block (capped 4-6 entries each + a `source` tag) when the mon is in the index; gracefully falls through to base-only response when not. Tool description updated to mention the new field. **Expected impact:** the deepseek run made 14+ pokedex calls plus several follow-up `search("Pokemon X usage")` queries — those follow-up searches should largely become unnecessary now since the pokedex result already includes the meta context.
- **R3 — Strip wasteful "Pokemon Champions"/"Regulation M-A" prefix at search tool layer.** New `stripChampionsPrefix()` regex in `executeSearch()`: anchored, case-insensitive, handles "Pokemon Champions ", "Champions ", "Reg M-A ", "Regulation MA ", "PoChamp " variants and combinations. Strip happens BEFORE embedding so the BGE encoder operates on the meaningful tail. If strip would yield <3 chars, falls back to original (defensive). System-prompt also nudges models away from the prefix at search-tool description. **Expected impact:** ~10-20% query embedding token savings when models still emit the prefix; cleaner embeddings = potentially better retrieval since the prefix words ("Pokemon Champions Regulation") are strong stopwords for the index.
- **Verification.** `npx tsc --noEmit` clean. `npx eslint src/lib/tools.ts src/lib/system-prompt.ts` clean. Smoke test [scripts/smoke-pokedex-enrich.ts](../scripts/smoke-pokedex-enrich.ts):
  - 89 Pikalytics rows loaded ✓
  - Froslass enriched (rank 19, 10% usage) ✓
  - Mega Froslass falls through to base form for usage lookup ✓
  - Krookodile not in pikalytics (below cutoff) → no `competitive` block, no error ✓
  - Strip regex passes 6 test cases (correct strip + correct no-op) ✓
- **Side-discovery → A14 filed.** Smoke test on Froslass returned moves like `Bora`, `Protezione`, `Palla Ombra`, `Velaurora`, `Sostituto`, `Congiura` — Italian. The current `pikalytics_usage.csv` Froslass row is in Italian. **A6's non-EN detector regex `[^\x00-\x7f]` misses ASCII-Latin scripts** (Italian, Spanish, French, Portuguese, German without diacritics). The detector was designed for JP/CN/Korean/Russian; it has no signal for Romance languages on common words. **A14 fix candidate:** cross-check scraped move names against canonical English `moves.csv` and trigger retry/fallback when >50% of moves don't match. Not blocking — the R2 enrichment mechanism is structurally correct; the data quality is a separate (already-known) class of issue.
- **What ships to users.** Pokedex calls return richer data, so models should make fewer redundant searches → lower latency + fewer tokens per request. Search queries are leaner → cleaner embeddings + token savings. Citations should be more reliable → fewer retry cycles. The system prompt no longer lies to models about chunk_id formats. Combined effect should be ~10-20% latency reduction on typical team-build runs once we re-test.

### Tier D Phase 2 (D7+D9 combined + D8) — SHIPPED (2026-04-25 evening)

- **Status:** SHIPPED. Three UX gaps surfaced during the 4-model real-world team-build compare (gemini-3-flash, gemma-4-26b, gemini-3-flash retry, deepseek-v4-flash) — all addressed before continuing model testing per user request.
- **D7+D9 — Clear assistant content on `iter_start` (iter > 0).** Single 5-line edit in [src/app/team/page.tsx](../src/app/team/page.tsx) `applyEvent`. When a non-first iteration starts, the assistant message's `content` field is wiped before the new iter's content streams in. Tool calls (`toolIds`) preserved since they represent real history. **Solves both bugs in one shot:**
  - **D7 (citation-retry duplicate):** when route.ts emits `citation_retry` then a fresh iter, the new iter's content replaces the failed iter's content instead of appending. No more "Snow-Sand-Pivoting" stacked on "Snow-Rain-Pivoting" in the same bubble.
  - **D9 (intermediate chain-of-thought accumulation):** for models that stream analytical prose between tool-calling iters (DeepSeek V4 Flash emitted ~6.8k chars across iters 0-10 before the iter-11 final synthesis), the user only sees the most recent iter's output. The "thinking out loud" doesn't pile up in the chat bubble.
- **D8 — Stop / Cancel button on `/team`.** `useRef<AbortController | null>` added; `streamReply()` creates a fresh controller per request and passes `signal` to fetch. New `cancel()` function calls `abortRef.current?.abort()`. Send button (Send icon) swaps to a destructive Stop button (`<Square>` icon, `variant="destructive"`) while `isStreaming === true`. AbortError caught silently in the catch block — sets `errorStage: "cancelled"` instead of bubbling as a transport error. Critical for prod: Vercel Lambda timeout is 5-10 min depending on plan and the DeepSeek V4 Flash run was 11.3 min — without cancel UX, the user would have been stuck waiting for either timeout or completion.
- **Verification.** `npx tsc --noEmit` clean. `npx eslint src/app/team/page.tsx` clean. Manual smoke pending user — recommend: (a) submit a query and click Stop mid-stream, (b) re-run any model and confirm the citation_retry no longer produces duplicate output, (c) test DeepSeek V4 Flash again and confirm only the iter-11 synthesis is visible.
- **What ships to users.** A reliable cancel path (no more 11-min hostage situations), clean retry UX (no duplicate output blocks), and a chat bubble that stays focused on the latest model output rather than accumulating chain-of-thought noise. Three failure modes that would otherwise compound on every long-running or chatty model are now structurally addressed.
- **Filed RAG suggestions for later (R1/R2/R3 in master plan).** Not shipped — user wants to focus on UX first. R1: audit chunk_id format heterogeneity (5+ patterns observed across runs all passed validation; either validator is too permissive or the index has chaotic IDs). R2: enrich `pokedex` tool output with usage stats inline (could cut ~30% of follow-up searches). R3: strip "Pokemon Champions Regulation M-A" prefix from search queries (wasted embedding tokens — every chunk IS Champions/M-A).

### Real-world `gemini-3-flash` run observed — first prod-shape data on an unbenchmarked model — 2026-04-25 afternoon

- **Status:** Observation only (no code change). User submitted a Snow-Balance team-build query through the freshly-Tier-D-shipped `/team` UX on `gemini-3-flash`. Run ID `exo6sh`. Captured from `npm run dev` logs.
- **Why this matters.** The 2026-04-23 7-model bake-off tested **Gemini 2.5 Flash Lite** (10/13 pass, 20% citations, "fast + chaotic") but never `gemini-3-flash`. This is the first prod-shape datapoint on the newer Gemini tier. User will follow up with the same query on the Gemma default for an apples-to-apples compare; this entry is the partial half.
- **Observed behavior.**
  - **Latency / shape:** 21.3s end-to-end, 4 iterations, 25 total tool calls. iter 0 = 6 (3 search + 3 pokedex), iter 1 = 9 (5 search + 4 pokedex), iter 2 = 8 (2 calc + 6 validate_set), iter 3 = 2 (1 pokedex + 1 validate_set), iter 4 = 0 tools / 4882 chars / `finish=stop`. Aggressive parallel fan-out — markedly higher tool fan-out per iter than Gemma's typical 2-3.
  - **Citations: 8/8 valid, 0 invalid, 0 retries.** Log: `[team exo6sh] citation_result valid=true cited=8/8 invalid=0 retry=false`. A4c worked perfectly on a model never seen by the bake-off — strong validation that the structural fix is model-agnostic.
  - **Items: in-roster, no hallucination.** All 6 picks (Choice Scarf, White Herb, Black Glasses, Leftovers, Sitrus Berry, Froslassite) verified against `items.csv`. Notable because [project_gemini3_eval.md](../.claude/projects/C--Users-paulo-Documents-LOCAL-WORKSPACE-1-pokemon-skill/memory/project_gemini3_eval.md) flagged gemini-3-flash for banned-item hallucination as a default-model blocker — this run did not exhibit it.
  - **Strategic claims verified against index.** Snow archetype matches canonical template ([data/knowledge/team_archetypes.md](../data/knowledge/team_archetypes.md) "Template 5 — Snow"). White Herb → Intimidate → Unburden → ~334 Speed Sneasler verified in [speed_tiers.md](../data/knowledge/speed_tiers.md). Mega Froslass Snow Warning verified in [mega_abilities.csv](../mega_abilities.csv).
  - **Roster deviations from canon.** Picked Krookodile + Rotom-Wash instead of canon's Palafin + Hisuian Arcanine. Defensible (second Intimidate, Charizard-Y answer) but worth flagging if we ever want stricter template adherence in the prompt.
- **Caveat — initial mis-diagnosis.** First read flagged the chunk_ids in the team output as malformed `[name](http://name)` markdown autolinks (citation hallucination class). Wrong call: the validator confirmed 8/8 valid, and the same `[](http://...)` pattern appears in the dev server's stale-index warning text on file paths (`memory-bank/[progress.md](http://progress.md)`). Conclusion: the rendering is the user's terminal / copy-paste autolinking file paths, not model output. Real chunk_ids in the JSON are clean. Lesson: when a citation looks wrong, check `citation_result` SSE event before blaming the model.
- **Side discovery — indexer ingest scope.** Stale-index warning at run-time named `memory-bank/progress.md`, `memory-bank/activeContext.md`, `memory-bank/rag-master-plan.md` — the indexer is pulling memory-bank meta-docs into chunks. Two costs: (a) noise in retrieval (planning docs aren't user-content), (b) spurious "stale" warnings every time we update the plan. Filed as new C4 in master plan; see roadmap row for fix path.
- **Compare — Gemma 4 26B run (2026-04-25 evening).** User submitted the same/similar Froslass+Krookodile prompt on `gemma-4-26b`. Three runs captured (`xsy7lm`, `aw0u5a`, `0yfbo1`):
  - **`xsy7lm` — failed.** Iter 1 crashed with `Bad tool_use JSON: unparseable after 12 frag(s)` (streaming tool_use JSON accumulation error). Tokens fragmented mid-`"Krookodile"` argument — known Gemma streaming flake (already documented in [project_gemma_agentic_quirks.md](../.claude/projects/C--Users-paulo-Documents-LOCAL-WORKSPACE-1-pokemon-skill/memory/project_gemma_agentic_quirks.md)). User-visible: the new D2 retry button is the right UX safety net for this exact failure mode — validates D2 wasn't theoretical.
  - **`aw0u5a` — succeeded after Gemma's own internal retry.** 58.8s, 4 iters, 11 tool calls (3+5+0+0 plus the failed iter), 4964 chars, **3/3 citations valid**. `parseErr=no-block` on iter 2 → citation_retry fired → iter 3 produced the claims block correctly. A4c worked.
  - **`0yfbo1` — second succeeded run** (likely follow-up question). 59.5s, 3 iters, 9 tool calls, 5724 chars, **2/2 citations valid, no retry needed**.
- **Side-by-side compare table** (gemini-3-flash `exo6sh` vs gemma-4-26b `aw0u5a`):

  | Metric | gemini-3-flash | gemma-4-26b | Δ |
  |---|---|---|---|
  | End-to-end latency | 21.3s | 58.8s | **2.8× slower** |
  | Iterations | 4 | 4 | tie |
  | Tool calls | 25 (6+9+8+2) | 11 (3+5+...) | Gemini fans out 2.3× harder |
  | Final synthesis iter | 8.1s / 4882 chars | 50.8s / 4964 chars | Gemma's synthesis is the latency hog |
  | Citation count | 8/8 valid | 3/3 valid | Gemini cites more aggressively; both 100% valid |
  | First-attempt success | yes | no (1 streaming crash, 1 missing block) | Gemma needed two retry mechanisms to land |
  | Banned items in JSON | 0 | 0 | tie clean |

- **Strategic finding.** Two prod-shape data points so far suggest **gemini-3-flash may be a viable paid-default candidate** if the bake-off were re-run on this newer Gemini tier (the original A1-alt only tested 2.5 Flash Lite). Faster, more reliable single-attempt completion, no banned-item hallucination on this run. **Caveat:** [project_gemini3_eval.md](../.claude/projects/C--Users-paulo-Documents-LOCAL-WORKSPACE-1-pokemon-skill/memory/project_gemini3_eval.md) flagged banned-item hallucination as the original disqualifier — a single clean run isn't enough to flip that. Would need a 3-run replay of the 13-test agentic suite to know if A4c + the phantom interceptor + content enrichment closed the gap. **Not a default-flip recommendation today** — just a "worth scheduling A1-alt-2 if user wants a faster default" data point.
- **Mention but not on-team caveat.** Gemma's prose mentioned **Safety Goggles** as a Krookodile alternative item ("Choice Scarf or Safety Goggles if needed"). Safety Goggles is in CLAUDE.md's MISSING ITEMS list. The actual `team-json` block uses Choice Scarf, so the recommendation is clean — but the *prose mention* of a banned item is the kind of thing a CLAUDE.md scanner would flag. Filed as low-priority observation; not a regression because nothing landed on the team.
- **A4c works on Gemma here too** — citation_retry fired once with `parseErr=no-block` (Gemma forgot the claims block, the retry nudge prompted it back), final 3/3 valid. Confirms A4c handles both the original "invalid IDs" failure mode AND the "no block at all" failure mode.
- **Indexer scope still bleeding** — the stale-index warning grew from 3 files to 5 between runs (`pikalytics_usage.csv`, `tournament_teams.csv` joined the memory-bank trio). Two of those are real-content (good freshness signal — Pikalytics + Sheets cron fired between runs), three are meta-docs that shouldn't be tracked. C4 still applies; arguably promote it to higher priority since the noise is now ~60% real content + 40% meta noise.
- **Compare — DeepSeek V4 Flash run (2026-04-25 evening, after model-list expansion).** Run `umbby1`. Same Snow-Balance prompt. **Latency 11 min 15s** (675s) — 32× Gemini-3-flash, 11× Gemma. 12 iterations, 45 tool calls (most thorough run by far), final synthesis iter alone took 3.5 min for 8748 chars. **12/12 citations valid first try (no retry).** Three-way compare:

  | Metric | gemini-3-flash | gemma-4-26b | deepseek-v4-flash |
  |---|---|---|---|
  | Latency | 21.3s | 58.8s | **675.6s (11.3 min)** |
  | Iterations | 4 | 4 | 12 |
  | Tool calls | 25 | 11 | **45** |
  | Output chars (final) | 4882 | 4964 | **8748** |
  | Citations | 8/8 | 3/3 (after retry) | **12/12 first try** |
  | Damage calcs in run | 0 | 0 | **13** |
  | Self-corrections observed | 0 | 0 | **2 (item clause, Rotom no-Hydro-Pump)** |
  | Bring-4 variants in output | minimal | minimal | **5 opponent strategies** |
  | Mega-option comparison | no | no | **yes (3 alternatives)** |

- **Strategic finding — DeepSeek is the most thorough but glacial.** Three behaviors stand out:
  1. **Self-validation:** caught its own item-clause violation (2× Focus Sash + 2× Sitrus Berry) and re-ran `validate_set` with adjusted items. Caught that Rotom-Wash doesn't learn Hydro Pump in Champions and pivoted the slot to Milotic. No other model exhibited this self-correction loop.
  2. **Damage-calc grounding:** ran 13 `calc` tool calls and embedded specific %HP figures in the output prose ("Blizzard 275-323%", "EQ 87-103%"). Other models produced strategic claims without quantitative backing.
  3. **Streamed reasoning between tool iters:** emitted 66+257+599+51+1931+309+1583+1079+176+307+415 chars of analytical prose across iters 0-10 before the iter-11 final synthesis (~6.8k chars of intermediate reasoning). This is unique behavior — Gemini and Gemma emit only at the synthesis iter. Combined with the existing D7 retry-duplicate-content bug, this means **the user's chat bubble accumulates ~15.5k chars across the run** (intermediate thinking + final synthesis), most of which is mid-process "thinking out loud" not meant for the final answer.
- **UX gaps surfaced by 11.3-min runs (proposed Tier-D follow-ups before more model testing):**
  - **D7 — citation-retry replaces, doesn't append** (already identified from gemini-3-flash run `ywu6i5`). Compounds with multi-iter streaming.
  - **D8 — Stop / Cancel button** in the team page header. 11-min runs without a cancel are a UX dead-end if the user wants to abort. Vercel Lambda timeout is ~5-10 min depending on plan; on prod, this run would have hit timeout.
...
- **Status:** SHIPPED. First active-track work after the 2026-04-25 strategic pivot declared the RAG roadmap feature-complete (see [rag-master-plan.md](rag-master-plan.md) "Strategic pivot (2026-04-25)" section). Three highest-ROI UX improvements bundled in one session because they share a single hot file ([src/app/team/page.tsx](../src/app/team/page.tsx)) and one new shared primitive ([src/components/ui/field.tsx](../src/components/ui/field.tsx)).
- **Rationale.** Webapp survey identified the team page as the only complex interactive surface and surfaced three concrete gaps: (1) debug sidebar `lg:flex`-only — phone users lose tool-call visibility; (2) silent transport errors with no retry path + blank assistant bubbles when stream stalls; (3) raw HTML inputs on `/calc` with no shared label/error primitive across pages. Bundling avoids three separate page-edit churn cycles.

- **D1 — Mobile-friendly Team debugger.** Extracted the existing aside contents into a new `<DebugPanel>` component in [src/app/team/page.tsx](../src/app/team/page.tsx) so the debug UI renders in two surfaces: the original desktop inline `<aside className="hidden ... lg:flex">` and a new `<Sheet side="bottom">` bottom drawer (`lg:hidden`, `h-[85vh]`) toggled via a "Debug" Button in the header (also `lg:hidden`). The header button shows an animated green pulse dot when a run is active so users can tell something is happening without opening the panel. Reuses [src/components/ui/sheet.tsx](../src/components/ui/sheet.tsx) base-ui primitive — no new dependencies.

- **D2 — Async UX hardening.** Refactored the streaming loop in `submit()` into a standalone `streamReply(apiMessages, assistantId)` helper so both submit and a new `retry()` function share the fetch/SSE/error/finally machinery. `retry()` drops the empty assistant placeholder, appends a fresh placeholder, and re-issues against the prior message history. Wired a "Retry" Button into the transport-error card. New `<ThinkingSkeleton>` (3 animated bars) renders inside the assistant bubble when streaming + content empty + no tools fired (signals "the request was sent, model is loading"). New "Stream ended without output. Try resubmitting your question." fallback text renders when stream finishes but assistant content is still empty (catches the rare case where the LLM returns no content + no tool calls). MessageView gained `isStreaming` and `isFinal` props derived from `idx === messages.length - 1`. Pre-existing `Date.now()` purity bug in `HistorySection` (line 822) fixed as a side-effect of the lint pass — `r.endedAt ?? r.startedAt` is safe since `streamReply`'s finally block always sets `endedAt` before pushing to `runs[]`.

- **D3 — Form & a11y baseline.** New shared primitives in [src/components/ui/field.tsx](../src/components/ui/field.tsx): `Field` (context-providing wrapper with `useId()`-derived id + `errorId` + `hintId`), `FieldLabel` (auto-binds `htmlFor` from context), `FieldHint`, `FieldError` (role="alert", auto-hides when empty), `useFieldControl()` hook returning `{id, aria-invalid, aria-describedby}` for non-Field-aware inputs to spread. Hint/error presence is tracked in context so `aria-describedby` only links live IDs (no orphan aria refs). Applied to:
  - [src/app/calc/calc-form.tsx](../src/app/calc/calc-form.tsx) — five fields (Attacker, Defender, Move, Weather, Spread checkbox) get explicit `useId()` IDs + `<Field>`/`<FieldLabel>` wrappers; weather select gets focus-visible ring; spread checkbox gets explicit `htmlFor` + focus ring; Move field gets `<FieldHint>` for the "optional" copy that was previously inline label text; form-level error card gets `role="alert"`; form gets `noValidate` so HTML5 messages don't fight our error UI.
  - [src/app/search/page.tsx](../src/app/search/page.tsx) — search Input gets explicit id, sr-only `<label>`, and `aria-label`.
  - [src/app/team/page.tsx](../src/app/team/page.tsx) — chat input gets id, sr-only `<label>`, `aria-label`, focus-visible ring, and disabled opacity. Send Button gets `aria-label="Send message"`. Model `<select>` and Debug Button got `aria-label`s in D1's edit. Form gets `aria-label="Team builder chat"`. All decorative icons (`<Send>`, `<Bot>`, `<User>`, `<AlertCircle>`, `<Wrench>` inside skeletons, etc.) get `aria-hidden`.

- **Verification.**
  - `npx tsc --noEmit` clean both mid-session and post-D3.
  - `npx eslint src/app/team/page.tsx src/app/calc/calc-form.tsx src/app/search/page.tsx src/components/ui/field.tsx` clean post-fix (initial run flagged unused `FieldError` import and the pre-existing HistorySection impurity; both addressed).
  - No retrieval / agentic eval impact — purely a UX/UI surface change.
- **What ships to users.** Phone users can now access debug tooling via a header button. Transport failures get a one-click retry instead of a stuck conversation. Assistant bubbles show progress and recovery copy instead of going blank. Three pages (calc, search, team) get screen-reader labels + visible focus rings + `role="alert"` errors. The new Field primitives unblock D-track future work (e.g. real per-field validation in calc, accessible model picker on team).
- **Rollback.** Revert this commit. The aside-only debug + raw-input forms restore. The `lib/phantom-guard.ts`-style "preserve interceptor in git history" approach also applies to `field.tsx` if a later session swaps in a third-party form library.
- **Follow-ups (not yet scoped):** mobile bottom-nav consistency across pages; `/pokedex` search-as-you-type; `/sets` filtering by core/archetype; design-token formalization (Tailwind v4 `@theme` block currently has no semantic color names like `--color-success`).

### C-tier housekeeping — drop dormant reranker code (B1 permanently deferred) — SHIPPED (2026-04-23 late evening) · commits `cd66ce1` + `9b85dfd` + `a1be19f`

- **Status:** SHIPPED. Master plan B1 row flipped from DEFERRED to PERMANENTLY DEFERRED; Part C bullet "Dead code to consider dropping" flipped to SHIPPED in the same commit. This closes out two weeks of on-and-off reranker work with a clean decision: no reranker in production, all plumbing removed.
- **Rationale.** Phase 3 attempted two reranker backends (Gemma 4 26B pointwise via OpenRouter, BAAI/bge-reranker-base via HF Inference). Both regressed matchup nDCG 15-18% under the old executor because Stage 6.3's sub-query-level reranking interfered with the boost layer. Phase 5 structurally unblocked the reranker (executor now re-applies force-includes + boosts post-merge against the ORIGINAL query), but the expected +0.02 matchup gain vs the complexity tax (reranker client + executor plumbing + tuning iterations + flaky HF Inference uptime) was marginal. Keeping dormant code alive incurs ~230 LOC of maintenance surface + one extra env-var branch + progress-event misnomer (`rerank_end` kept firing even though the stage was boost+sort only). Verdict after 2 failed attempts: B1 is not worth revisiting; preserve only in git history.
- **Scope removed:**
  - **[lib/rerank.ts](../lib/rerank.ts)** — DELETED (342 LOC). Three clients: `rerankCandidates` (Jina v2, permanently no balance), `rerankWithGemma` (~140 LOC — OpenRouter pointwise, 10-slot inline worker pool, manual `AbortController` + `clearTimeout`, 800-char snippet, per-query SHA256 LRU cache), `rerankWithCrossEncoder` (~80 LOC — BAAI/bge-reranker-base via HF Inference `text-classification` pipeline, single batched HTTP call for all 40 candidates, 1500-char snippet). All returned `Map<id, score>` in [0,1] or null on failure. RERANK_POOL=40.
  - **[lib/rag.ts](../lib/rag.ts)** — RERANKER env-var dispatch block removed (~45 LOC). Rerank import line removed. Legacy `GEMMA_RERANK_ENABLED` env fallback removed. ProgressStage `"rerank_end"` renamed to `"boost_end"` (accurate — the stage was always boost+sort). Both inline `boostMul` usage and the executePlan `boostMul=1` positional arg dropped.
  - **[lib/rag/boost.ts](../lib/rag/boost.ts)** — `applyBoosts` signature `(candidates, intent, route, question, boostMul)` → `(candidates, intent, route, question)`. Return value `r.score + boost * boostMul` → `r.score + boost`. Header comment rewritten to note the historical presence of `boostMul` + how to re-introduce it if a future reranker ships.
  - **[lib/query-executor.ts](../lib/query-executor.ts)** — `executePlan(...boostMul: number = 1)` param dropped. `applyBoosts(..., boostMul)` in the executor → 4-arg signature. Comment rewritten to point at the correct re-introduction path (wire reranker between `collectForceIncludes` and `applyBoosts` + plumb `boostMul` through).
  - **[src/app/team/page.tsx](../src/app/team/page.tsx)** — stage-color map `case "rerank"` → `case "boost"`. Green color unchanged.
- **Memory-bank updates in the same session:**
  - **rag-master-plan.md** — B1 row PERMANENTLY DEFERRED; A4 section's "Citation hallucination" paragraph rewritten to A4c SHIPPED; Expected trajectory table gained A4c + cleanup rows; Part C "Dead code" bullet rewritten as SHIPPED.
  - **techContext.md** — Multi-signal boosting section renamed; 15 categories; A7 core-WR NL boost explicitly listed; Reranker dispatch paragraph rewritten as "permanently deferred".
  - **systemPatterns.md** — File-layout stripped of `lib/rerank.ts`; RAG pipeline step 12 renamed "Merge + Boost"; 15 boost categories; `boost_end` progress event.
  - **activeContext.md** — SHIPPED list gained A4c + cleanup rows; next-actions list trimmed (only A8/A9/A4b remain, all optional/low-priority); key-code pointers updated.
- **Verification (full cleanup gate):**
  - `npx tsc --noEmit` clean.
  - `npx tsx scripts/search.ts "Protect PP in Champions" 3` — canonical Phase 4 sanity returns `moves.csv` rank 1 (0.0336), `champions_rules.md` rank 2 (0.0328), `damage_calc.md` rank 3 (0.0320). Identical order to pre-cleanup baseline.
  - `npx tsx scripts/search.ts "Archaludon Pelipper rain core win rate" 3` — A7 smoke returns `meta_snapshot.md` ranks 1/2/3 at 0.1377/0.1322/0.1309. Identical scores to immediate-post-A7-ship (confirms A7's +0.08 boost still fires cleanly under the simplified `applyBoosts` signature).
  - 100-case retrieval eval: **overall nDCG 0.844** (-0.1% vs 0.845 post-A3 baseline). Per-intent all within ±0.005. Adversarial 0.686 (gate ≥0.68 ✓). Deterministic across two back-to-back runs (same exact numbers). Canonical snapshot: [retrieval-2026-04-24T05-27-53-696Z.json](eval-baselines/retrieval-2026-04-24T05-27-53-696Z.json).
  - 1-run agentic eval (`gemma-4-26b --real-rag`): **13/13 pass, 100% citation validity** (5/5 retrieval tests, 41/41 chunk_ids valid, 8 retries fired and all succeeded). Tokens/pass elevated to 56852 (vs 25-30k recent baseline) — environmental, the redundant 2nd retrieval eval was running concurrently competing for Supabase RPC time. Hard gates (pass + citation) cleared. Snapshot: [model-eval-2026-04-24T05-24-06.json](../snapshots/model-eval-2026-04-24T05-24-06.json).
- **Rollback.** Revert commits `cd66ce1` + `9b85dfd` + `a1be19f`. The three reranker clients are preserved in git history at `lib/rerank.ts` pre-`cd66ce1`. Re-introduction path (if a future workload justifies): (1) rebuild clients from history, (2) wire reranker step in [lib/query-executor.ts](../lib/query-executor.ts) between `collectForceIncludes` and `applyBoosts` scoring the merged pool against `plan.originalQuery`, (3) add `boostMul: number` back to `applyBoosts` signature and `executePlan` signature, (4) add env-var dispatch back in [lib/rag.ts](../lib/rag.ts) orchestrator. Expect ~2 hours to reconstruct from this note + git history.
- **What ships to users.** No user-visible change. RAG behavior identical (verified by retrieval + agentic evals). 230 LOC less code to maintain, one less env-var surface, one less misleading progress event name.

### A4c — Citation retry nudge enumerates valid chunk_ids — SHIPPED (2026-04-23 late evening) · commit `2ba58b5`

- **Status:** SHIPPED. Addresses the last of four user-visible quality levers named in the master plan strategic reframe: "Gemma occasionally fabricates chunk_ids even after the Phase 2 auto-retry nudge." Prior 3-run citation-validity baseline: 80/100/80. Post-A4c 3-run: **100/100/100**.
- **Root cause.** The existing [formatValidationNudge](../lib/validate-citations.ts) told the model which IDs were invalid but not which IDs were valid. After the nudge, Gemma had to remember the valid IDs from earlier tool results — unreliably. Closing the guess loop by explicitly enumerating valid IDs in the retry nudge body should give Gemma a pick-one-from-this-set instruction instead of a try-again-from-memory one.
- **Implementation (single-file core edit + 2 callers):**
  - **[lib/validate-citations.ts](../lib/validate-citations.ts)** — `formatValidationNudge(invalidIds: string[])` signature extended to `(invalidIds: string[], seenChunkIds: Set<string>)`. Added helper `formatValidIdsList(seenChunkIds)` with `MAX_VALID_IDS_IN_NUDGE=50` cap (~1k tokens of surface — typical turns see 10-25 chunk_ids; cap prevents long conversations from blowing the retry-nudge budget). Nudge body gained a trailing markdown block: `**Valid chunk_ids you received this conversation — you MUST pick from this set:** \`id1\`, \`id2\`, ...` (with "(+N more)" tail when truncated). Both the invalid-IDs branch and the empty-claims branch get this appended block.
  - **[src/app/api/team/route.ts](../src/app/api/team/route.ts)** — `formatValidationNudge(v.invalidIds)` → `formatValidationNudge(v.invalidIds, seenChunkIds)`. The `seenChunkIds` Set was already in scope (accumulated by the agent loop across all `search` tool calls).
  - **[scripts/eval-models.ts](../scripts/eval-models.ts)** — same signature update.
- **Gate (3-run gemma-4-26b --real-rag):**
  - Pre-A4c baseline: 12/13 + 13/13 + 12/13 pass, citation validity 80%/100%/80%, tokens/pass ~30k.
  - Post-A4c: **12/13 + 13/13 + 13/13 pass, citation validity 100%/100%/100%**, tokens/pass 30334 / 26248 / 25014 (avg ~27k — best Gemma result measured to date; the retry-nudge enumeration adds tokens but the model needing fewer retries net-reduces total token count). Retries fired: 8 / 7 / 5 across the three runs. All retries succeeded (0 final invalid IDs).
  - Run 1's single fail is `item_availability` — content check ("No valid Champions items mentioned in response"), unrelated to citations; pre-existing flake class (same as the occasional `team_json` "forgot JSON block" flake).
  - Snapshot files: [model-eval-2026-04-24T04-20-31.json](../snapshots/model-eval-2026-04-24T04-20-31.json), `...04-28-39.json`, `...04-34-11.json`.
- **Why 50 IDs.** Typical conversation has 10-25 valid IDs across all search calls. 50 is a safety cap — if truncation ever fires, the retry nudge's effectiveness degrades gracefully (the tail "...+N more" signals to the model there are more options). `MAX_VALID_IDS_IN_NUDGE` lives as a named constant so future tuning has a handle.
- **Rollback.** Revert commit `2ba58b5`. Two callers revert with it. Phase 2's existing "nudge once per request" rate-limit logic is unchanged.

### A12 — Serebii scraper on weekly cron (patch-safety net) — SHIPPED (2026-04-23 late evening) · commit `cc7e927`

- **Status:** SHIPPED. Master plan A12 row flipped from "NEXT (low urgency)" to SHIPPED. Last manual-only scraper in the data pipeline now automated — closes the last gap from the 2026-04-23 freshness audit (A10/A11/A12/A13).
- **Rationale.** Serebii publishes static game data (Pokemon rosters, moves, items, abilities, Mega stats). It only drifts when Nintendo patches Pokemon Champions. Pre-A12, `scraper.py` was manual-only — a patch drop could silently leave `pokemon_champions.csv` / `mega_evolutions.csv` / `moves.csv` stale for days or weeks until someone noticed. Weekly cron is the right cadence: high-frequency would be wasteful (data rarely changes) AND risk tripping Serebii's Cloudflare rate limits (the scraper already honors 1s/page politeness = ~3min/run for 186 Pokemon pages).
- **Architecture: separate workflow file** (not an extra step in `refresh.yml`, not a second job). Trade-off documented in the approved plan at `~/.claude/plans/plan-out-the-next-misty-tower.md`:
  - Separate `.github/workflows/refresh-serebii.yml` is cleanest — independent cadence, clean separation, isolated disable/debug, separate concurrency group so a scraper regression doesn't mask the 2×/day Pikalytics+Sheets pipeline failures.
  - Rejected: extra step in `refresh.yml` (fires 2×/day — wasteful + rate-limit risk). Rejected: second job in `refresh.yml` with own cron (GH Actions applies all `schedule` crons to every job unless gated on `github.event.schedule` — adds awkward conditional plumbing).
- **Implementation — [.github/workflows/refresh-serebii.yml](../.github/workflows/refresh-serebii.yml)** mirrors `refresh.yml` structure:
  - `cron: "0 4 * * 0"` — Sunday 04:00 UTC (23:00 CST Saturday), after weekend tournament window; offset from 00:00/12:00 UTC Pikalytics+Sheets cron so logs don't interleave.
  - `concurrency.group: refresh-serebii` — isolated from the existing `refresh` group.
  - `timeout-minutes: 15` — Serebii scrape ~3min + reindex ~1min; generous headroom.
  - `environment: Production` — scopes `SUPABASE_SECRET` + `NEXT_PUBLIC_SUPABASE_URL`.
  - `permissions: contents: write` — auto-commit step needs push rights.
  - Steps: checkout → setup-node@v4 (node 22) → setup-python@v5 (python 3.12) → `npm ci` → `pip install requests beautifulsoup4 lxml` → `python scraper.py` (`continue-on-error: true` — known fragilities: Mega X/Y disambiguation, 21 FORM_VARIANTS hardcoded paths, Floette-Eternal layout edge case — see [errors.md](errors.md) rows 15-18, 36-37) → `npx tsx scripts/index-data.ts` with supabase `env:` block → auto-commit "refresh: weekly serebii scrape" + git push.
  - `workflow_dispatch` included for manual one-off patch checks.
- **Memory-bank updates in the same commit:**
  - **rag-master-plan.md** — A12 table row SHIPPED, A12 detail section rewritten with final architecture, Expected trajectory table gained A12 row.
  - **techContext.md** — Source inventory row 1 (Serebii) Current schedule updated; Current automation state list gained Serebii entry; Freshness SLOs table Serebii row "Current cadence" flipped to weekly + "Gap" closed.
  - **activeContext.md** — Next actions list dropped A12; SHIPPED list gained A12 entry.
- **Gate (pending):**
  - YAML parse check + file structure validated (gh CLI would confirm after push; skipped locally since the user controls push).
  - Two-week observation gate: Sunday 2026-04-28 (first scheduled fire) + Sunday 2026-05-05 (second). Both should show green check + either auto-commit `refresh: weekly serebii scrape` (if Nintendo patched Champions in that week) or clean no-op "No data changes to commit". If both clean, close A12 as SHIPPED + verified.
  - Row-count sanity: baseline `pokemon_champions.csv`=216, `mega_evolutions.csv`=59, `moves.csv`=494, `items.csv`=138. Drift without explanation → investigate upstream restructure before merging.
- **Explicitly out of scope (C-tier backlog):**
  - Row-count diff logging / patch-detection notification in the workflow.
  - Retry logic for transient 429/503 (weekly cadence + `continue-on-error: true` is sufficient).
  - Shared setup-python pip-cache across workflows (optimization; scrape is <3min so not worth the complexity).
- **Rollback.** Delete the workflow file + revert the commit. Zero impact on the existing 2×/day Pikalytics+Sheets+reindex pipeline (separate workflow, separate concurrency group).

### A7 — NL "X+Y core WR" retrieval hardening — SHIPPED (2026-04-23 late evening) · commit `7ac3b1c`

- **Status:** SHIPPED. Master plan A7 row flipped from "NEXT" to SHIPPED. Closes the `/lookup` gap A5 self-eval exposed (meta_core_attribution test 13).
- **Problem.** `/lookup "Archaludon Pelipper rain core win rate"` pre-fix returned top-5 at similarity ~0.11 — all `team_archetypes.md` chunks, NOT the `meta_snapshot.md` top-cores table where the actual 55.8% WR lives. Reformulating as "top cores win rate Archaludon Pelipper Electro Shot" surfaced it at rank 3 / 0.078 — still low. Gemma's agent works around via query-planner decomposition, but direct `/lookup` users (CLI, or any less-capable agent) hit the bare gap.
- **Root cause — two combined:**
  1. The top-cores table chunk's embedding gets dominated by table-level vocabulary ("Top Cores by Win Rate") and loses the row-specific semantics (per-row Pokemon-name co-occurrences with WR% are scattered).
  2. Boost layer: archetype token "rain" in the query lifts 17 `team_archetypes.md` chunks via theory-route (+0.025) + archetype-match (+0.025) ≈ +0.05 vs `meta_snapshot.md`'s +0.020 knowledge-tier baseline. Content tweak alone (attempt 1 below) was insufficient — meta_snapshot still landed at ranks 17-20 post-content even after adding dense per-core prose.
- **Attempt 1 (content-only — insufficient):** Added a `### Core Win Rates (Natural Phrasing)` subsection in [data/knowledge/meta_snapshot.md](../data/knowledge/meta_snapshot.md) after the top-cores table. Six bullet points, each with dense per-core sentence using both "+" and "and" phrasings tightly coupling Pokemon names with WR% (e.g., "**Archaludon + Pelipper** rain core win rate: 55.8% at 20.8% usage. Archaludon Pelipper rain core fires Electro Shot instantly under Drizzle."). Reindex landed, but target query still returned team_archetypes at ranks 1-17. Distinctive-phrase search confirmed the new chunk was indexed (Result 2 at similarity 0.0761) but couldn't outscore the archetype boost stack.
- **Attempt 2 (code + content — works):** Added a +0.08 boost in [lib/rag/boost.ts](../lib/rag/boost.ts) for `meta_snapshot.md` chunks when query matches `\bcore\b` AND `\b(win rate|winrate|wr)\b`. Calibrated to clear the ~+0.07 team_archetypes theory+archetype+tier stack. Placed after the speed-tiers boost block; code note mentions the calibration rationale. Trigger is extremely narrow — zero golden-set cases match (verified by regex grep).
- **Gate met:**
  - `/lookup "Archaludon Pelipper rain core win rate" 3` returns meta_snapshot ranks 1/2/3 at 0.1377/0.1322/0.1309 (vs team_archetypes' 0.1114). The "Top Cores by Win Rate" chunk containing the 55.8% figure lands at rank 3 — exactly the gate spec.
  - Pre-fix: chunk absent from top-20.
  - `npx tsc --noEmit` clean.
  - 25-test RAG eval (`npx tsx scripts/eval.ts`) identical to baseline (22/25 pass, MRR 1.000, same 3 pre-existing fails in unrelated mechanic/item-lookup/stat-filter categories). Also verified via diffing the eval run WITHOUT the A7 changes (stash, re-run) — identical to WITH the A7 changes, confirming zero regression on the pre-existing 25-test set.
- **Why the boost is narrow.** The trigger pattern `\bcore\b AND \b(win rate|winrate|wr)\b` requires both tokens in the same query. Covers intended "X+Y core WR" formulations without affecting generic rain/sun/sand queries. Regex grep of the 100-case golden set finds zero matches → zero retrieval-eval impact.
- **Rollback.** Revert commit `7ac3b1c`. Content subsection reverts (prior version was bare table). Boost reverts to the 14-category pre-A7 pattern.

- **Status:** SHIPPED. Originally scoped in the master plan as "JP→English cleanup for 14 pikalytics rows" based on the A5 self-eval `awk` scan. Investigation across three consecutive scrapes reframed the problem to "multilingual per-URL cache flips" — 7 distinct locales observed (Japanese, Traditional Chinese, Simplified Chinese, Spanish, German, French, Korean) with the affected set drifting per scrape: run N hit Delphox/Tauros/Floette-Eternal (3 mons); run N+1 hit Ninetales-Alola/Gengar/Gyarados/Whimsicott (4 different mons); run N+2 hit 17 mons spanning all 7 locales. Master plan A6 row + errors.md row 47 both flipped to SHIPPED/RESOLVED in the same commit.
- **Root cause.** Pikalytics is behind Cloudflare; Cloudflare caches by URL and ignores the `Accept-Language` header the Phase 8 fix added. Each URL picks up whichever locale Cloudflare happened to cache first from an upstream visitor. Cache-bust query params defeat most of these; a few URLs have persistently non-English cache entries (e.g., Floette-Eternal).
- **Layered fix in [scraper_pikalytics.py](../scraper_pikalytics.py):**
  - Cache-bust query param `?_r=<random>` on every fetch; `Cache-Control: no-cache` + `Pragma: no-cache` headers. Forces Cloudflare to miss its cache on each call.
  - Post-parse non-ASCII detection via `NON_ENGLISH_RE = re.compile(r"[^\x00-\x7f]")`. Initially implemented as CJK-only (`[\u3040-\u309f\u30a0-\u30ff\u4e00-\u9fff]`), widened after Spanish/German/French/Korean slipped through. `moves.csv` + `items.csv` verified 100% ASCII so no false positives.
  - In-scrape retry loop `MAX_LANG_RETRIES=2` — each retry gets a fresh random cache-bust. Catches in-scrape regressions without leaving the function.
  - `load_prior_english_rows(path="pikalytics_usage.csv")` at scrape start loads existing CSV rows that are fully English. When the fresh scrape regresses to non-English AND retries fail, the scraper uses the prior English row instead. Critical resilience property: the CSV never regresses even when Pikalytics serves non-English.
  - Final assertion at end of `main()`: scans all result rows for remaining non-ASCII. If any, prints row names and `sys.exit(1)` — GH Actions surfaces the failure instead of silently publishing non-English data.
  - Manual English seed: Floette-Eternal's row pulled from commit `3555ad2` and written into the CSV because Pikalytics returns Chinese on every retry for that URL (cache appears permanently non-EN). The prior-EN fallback then preserves the seed on subsequent runs.
- **Verification (end-to-end):**
  - After 3rd scrape run: 17 rows regressed to non-EN, all 17 fell back to prior EN successfully; 1 remaining (Floette-Eternal) had no prior EN → seeded from `3555ad2`.
  - Post-seed CSV: 0 non-ASCII rows (verified with both `awk` and the Python regex `[^\x00-\x7f]`).
  - Full `--force` reindex green: 2,639 chunks (vs 2,511 baseline; growth from new transcripts landing during earlier A10 local runs).
  - `/lookup` spot-checks on previously-affected rows all return clean English top-1: Delphox ("Heat Wave, Protect, Psychic, Substitute, Encore, Nasty Plot, Psyshock, Dazzling Gleam..."), Tauros, Floette-Eternal (matches `3555ad2` seed), Charizard, Basculegion ("Last Respects, Wave Crash, Aqua Jet, Flip Turn, Protect, Liquidation, Icy Wind...").
- **What ships to users.** `pikalytics_usage.csv` with zero non-ASCII rows + a scraper that keeps it that way on every cron run, regardless of which locales Pikalytics' Cloudflare layer rotates next. Self-healing property: future cron runs where Pikalytics serves non-EN for different Pokemon will silently fall back to the prior-EN row without polluting the CSV.
- **Rollback.** Revert the single commit `6c2c101`. Prior-EN fallback is additive-only; no destructive schema changes. Seed row removal would just cause next scrape to serve Chinese for Floette-Eternal — not catastrophic since the UI would display Chinese, same failure mode as before the fix.
- **Observational gate (not blocking).** After the next 2 GH Actions cron fires (~24h @ `0 0,12 * * *`), confirm that auto-commit `refresh: auto-update competitive data` from `github-actions[bot]` still leaves `pikalytics_usage.csv` at 0 non-ASCII rows. The fallback logic runs in cron just like it does locally, so this should be automatic.

### A13 — Surface "last refreshed" staleness telemetry — SHIPPED (2026-04-23 evening) · commit `740ef9b`

- **Status:** SHIPPED. Master plan A13 row (was "NEXT (UX)") flipped to SHIPPED in follow-up commit (memory-bank doc pass). Implementation ~1.5h end-to-end; closes the feedback loop on the A10/A11 freshness work that shipped earlier the same session.
- **Goal.** Users on webapp + `/lookup` CLI see "Data refreshed N hours ago" with per-source breakdown (youtube, pikalytics, sheets, serebii, knowledge). Amber warning at >72h stale. Would have proactively caught the "YouTube not in cron" gap that A10 investigated reactively.
- **Implementation:**
  - **[lib/rag.ts](../lib/rag.ts)** — new exported `getStaleness(): Promise<StalenessInfo | null>` next to the existing `checkStaleness()`. Returns `{indexedAt, hoursSinceIndex, sources: StalenessSource[], hasFsDrift}`. `StalenessSource = {name, fileCount, mostRecentMtime, hoursSinceMostRecent, hasFsDrift}`. Source buckets derived from file path via `SOURCE_MAP`: `data/transcripts/*` → youtube, `pikalytics_usage.csv` → pikalytics, `tournament_teams.csv` → sheets, `data/knowledge/*` → knowledge, `memory-bank/*` → internal (filtered out), everything else → serebii. 60s in-process cache (`STALENESS_TTL_MS`) — amortizes one DB hit across multiple `query()` calls per request. On Vercel, fs drift detection is skipped (Lambda mtimes are build-time not reindex-time so every file looks "drifted"). New type `StalenessSourceName = "youtube" | "pikalytics" | "sheets" | "serebii" | "knowledge"`. `"staleness"` also added to `ProgressStage` union (forward-proof; not currently used for emit — route.ts uses its own top-level SSE event type).
  - **[src/app/api/team/route.ts](../src/app/api/team/route.ts)** — emits `send({type: "staleness", data: info})` once per request, right after the `meta` event, via a non-blocking `getStaleness().then(info => ...).catch(() => {})`. Choice: route.ts fires the single request-level event rather than threading through `onProgress` in `query()` (would have emitted N times per request under the agent loop).
  - **[src/app/api/team/health/route.ts](../src/app/api/team/health/route.ts)** — GET now also awaits `getStaleness()` and returns `{staleness}` in the response alongside `supabase`/`embed`/`providers`. This is how the webapp populates the footer on mount (before any query runs).
  - **[src/app/team/page.tsx](../src/app/team/page.tsx)** — new `<StalenessFooter>` component rendered below the input form in the chat column. Compact single-line button: "Data refreshed Nh ago"; amber-styled when max source age > 72h (`STALENESS_WARN_HOURS`); expand-on-click 3-column grid showing per-source (name, hours-ago, file-count + optional "drift" flag). State sources: (a) `probeHealth()` in the existing mount effect pulls initial staleness from `/api/team/health` response; (b) SSE handler updates state when agent loop emits a `staleness` event. Types mirrored locally (`StalenessSourceName`, `StalenessSource`, `StalenessInfo`) to avoid bundling server code. `React.Fragment` import added for the keyed per-source rows.
  - **[scripts/search.ts](../scripts/search.ts)** — at startup (after `getStaleness()`), prints one line: `Data refreshed Nh ago · sources: youtube=Nh, pikalytics=Nm*, serebii=Nh, sheets=Nm*, knowledge=Nh (* = local fs drift)`. Warn prefix `[STALE >72h]` if max source age exceeds 72h. Keeps the CLI output terse.
- **Formatting helper** — `formatHoursAgo(hours)` in both page.tsx and search.ts (duplicated; could be shared later): < 1h → "Nm", < 48h → "Nh", else → "Nd".
- **Verification:**
  - `npx tsc --noEmit` clean across all 4 touched .ts files.
  - CLI smoke test on `"Protect PP in Champions" 3` returns expected top-3 (moves.csv rank 1, champions_rules.md rank 2, damage_calc.md rank 3 — matches Phase 4 post-split sanity-check baseline).
  - Natural fs-drift surfaces immediately: pikalytics + sheets showed `*` markers (local git-pull mtimes newer than indexed mtimes from the CI scrape) — exactly the expected signal.
- **Rollback.** Revert single commit `740ef9b`. Zero coupling to RAG path — the staleness read is additive and runs in parallel with agent loop work.
- **Pre-A13 retrieval baseline unchanged** — staleness plumbing is a read-only side-channel, doesn't touch `query()` / planner / executor / boosts. `nDCG@10 = 0.845` post-A3 remains the reference.
...
- **Scheduled task registered (2026-04-23 evening, user go-ahead):** `schtasks /create /tn "pokemon-youtube-scraper" /tr "<repo>\scripts\scrape-youtube-local.bat" /sc hourly /mo 12 /ru paulo /it`. `schtasks /query` confirms Status=Ready, Schedule=Every 12 Hours, Run As User=paulo, Next Run=2026-04-24 07:57 local. `/it` (interactive) fits the desktop use case — fires only when user is logged on; no wake-timer.
- **Observational gate (A10 revised):** after 2 scheduled task fires (~24h, so by 2026-04-25 evening), confirm:
  - `data/transcripts/` contains new `.md` files with dates ≥ 2026-04-24.
