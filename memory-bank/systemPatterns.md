...
│   ├── rag.ts                  Hybrid search (pc_hybrid_search RPC) + intent classification + structured queries + force-includes + boosts + staleness (A13, 2026-04-23: getStaleness() returning StalenessInfo — per-source mtime ages surfaced via SSE in route.ts, health endpoint, search.ts CLI line). Reranker dispatch removed 2026-04-23 late evening (B1 permanently deferred; see git history for the three prior clients)
