...
_Last updated: 2026-04-23 late evening (post A6 multilingual fix + A13 staleness telemetry). Purpose: one-page "right now" snapshot. Forward plan lives in [rag-master-plan.md](rag-master-plan.md); stage-by-stage history in [progress.md](progress.md); bug log in [errors.md](errors.md); data-pipeline source-of-truth in [techContext.md § Data Pipeline](techContext.md#data-pipeline)._
...
- **A13 + A6 SHIPPED (late evening, commits `740ef9b` + `6c2c101`):** A13 surfaced staleness telemetry to webapp footer + `/lookup` CLI — new `getStaleness()` in [lib/rag.ts](../lib/rag.ts) returning per-source mtime ages (youtube/pikalytics/sheets/serebii/knowledge); SSE `{type:"staleness"}` event in [src/app/api/team/route.ts](../src/app/api/team/route.ts); `<StalenessFooter>` in [src/app/team/page.tsx](../src/app/team/page.tsx) with amber >72h warning + expand-on-click per-source grid; `/api/team/health` returns staleness so the footer renders on mount. A6 fixed multilingual locale flips in pikalytics scrape — scope wasn't JP-only (7 locales observed: JP/CN-trad/CN-simp/ES/DE/FR/KR) and set drifts per run, so fix is structural in [scraper_pikalytics.py](../scraper_pikalytics.py): cache-bust query param `?_r=<random>` + `Cache-Control: no-cache` + non-ASCII detection (`[^\x00-\x7f]`) + retry loop + `load_prior_english_rows()` fallback + `sys.exit(1)` on no-fallback + manual English seed for Floette-Eternal (permanently non-EN cache). CSV now 0 non-ASCII; scraper self-heals across cron runs. Reindex green @ 2,639 chunks; `/lookup` spot-checks clean English for Delphox/Tauros/Floette-Eternal/Charizard/Basculegion.
...
- **C-tier housekeeping: dormant reranker cleanup — SHIPPED.** B1 (Phase 3 reranker retry) permanently committed as deferred; 2 prior attempts regressed matchup 15-18%, Phase 5's structural fix only yields ~+0.02 expected gain — marginal ROI verdict stands. Dropped: [lib/rerank.ts](../lib/rerank.ts) (entire file — 3 reranker clients: Jina v2, Gemma 4 26B pointwise via OpenRouter, BGE cross-encoder via HF Inference), RERANKER env-var dispatch in [lib/rag.ts](../lib/rag.ts) (~45 LOC), `boostMul` plumbing through [lib/rag/boost.ts](../lib/rag/boost.ts) (param + multiplier), `boostMul` through [lib/query-executor.ts](../lib/query-executor.ts) (param + forwarding). Progress-event `rerank_end` renamed to `boost_end` (more accurate — stage is boost+sort); [src/app/team/page.tsx](../src/app/team/page.tsx) stage-color map updated `"rerank"` → `"boost"`. Net savings ~230 LOC + simpler orchestrator + one less env-var surface. Gate: tsc clean; canonical `/lookup "Protect PP in Champions" 3` returns moves.csv/champions_rules.md/damage_calc.md in expected order; A7 `/lookup "Archaludon Pelipper rain core win rate" 3` identical post-cleanup (0.1377/0.1322/0.1309 meta_snapshot 1-2-3). 100-case retrieval eval + 1-run agentic eval pending.
- **A4c (citation retry nudge hardening) — SHIPPED.** [lib/validate-citations.ts](../lib/validate-citations.ts) `formatValidationNudge(invalidIds, seenChunkIds)` signature extended to accept the valid-id Set; nudge text now ends with "**Valid chunk_ids you received this conversation — you MUST pick from this set:** ..." enumerating up to 50 valid IDs (≈1k tokens cap). Closes the model's guess loop — previously it knew which IDs were invalid but had to remember valid ones from earlier tool results. Callers updated in [src/app/api/team/route.ts](../src/app/api/team/route.ts) and [scripts/eval-models.ts](../scripts/eval-models.ts). Gate: 3-run gemma-4-26b --real-rag citation validity **80/100/80 → 100/100/100**. Pass rate 12/13, 13/13, 13/13 (Run 1 fail is `item_availability` content check, unrelated to citations). Best Gemma tokens/pass ever: 30334/26248/25014 (avg ~27k vs prior ~30k baseline). tsc clean. Snapshots: `snapshots/model-eval-2026-04-24T04-{20-31,28-39,34-11}.json`.
...
A1/A2/A3/A4/A4c/A5/A6/A7/A10/A11/A12/A13 all shipped this session. Remaining Tier A (in priority order):
