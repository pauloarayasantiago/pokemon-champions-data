...
| A16 | Perfect system prompt — dedicated session with research | A | **PLANNED — needs focused session** | User-flagged 2026-04-25 evening: block out a dedicated session, not piecemeal. Scope includes prompt-engineering research (tool-using-agent best practices, citation hygiene patterns, system-prompt patterns from comparable products) BEFORE editing. Target: tighter rules in [src/lib/system-prompt.ts](../src/lib/system-prompt.ts) (currently `2026-04-25.v4.5-reemit-both`), fewer model-specific quirks, better eval scores across the dropdown. Aligns with [§ Strategic reframe (2026-04-23) item 3 "Agent ergonomics"](#strategic-reframe-2026-04-23). Coordinates with anyone else queuing prompt edits — touches the same file as v4.5 |
