...
| A10 | YouTube scraper on local cadence (cloud-ban-aware hybrid arch + whisper fallback waterfall) | A | **SHIPPED-then-DEGRADED-then-RESOLVED (2026-04-23 → 2026-04-25)** | Initial ship: cron step (`Saved: 0` due to cloud-IP ban) → reverted. Local hybrid via Windows schtasks @ 12h: 2026-04-23 smoke landed 22 transcripts (`dfc3664`), but every fire 2026-04-24+ returned `Saved: 0` — YouTube extended rate-limiting to residential IPs at the captions endpoint. 2026-04-25 fix is **two-method per-video waterfall**: primary [scraper_youtube.py](../scraper_youtube.py) `get_transcript()` uses `yt-dlp --write-auto-subs --impersonate Chrome`; on failure, fallback [scraper_youtube_whisper.py](../scraper_youtube_whisper.py) downloads audio via yt-dlp `-x` (different YouTube infrastructure, NOT rate-limited) and transcribes locally with faster-whisper `medium.en` on RTX 2070 SUPER (~10× realtime). Plus `DELAY_SECONDS=5`, `MAX_FETCH_PER_RUN=20`, `MAX_WHISPER_PER_RUN=5`, `RATE_LIMIT_STREAK_ABORT=3` (counts streak across both methods), schtasks retuned daily 03:00. End-to-end verified 2026-04-25 21:30: ytdlp 429 → whisper saved 36 KB transcript. Frontmatter `source: ytdlp\|whisper` for downstream audit. Detail in [errors.md](errors.md) + [progress.md](progress.md). |
...
| A16 | Perfect system prompt — dedicated session with research | A | **EXECUTED — PROMPT-ONLY EXHAUSTED (2026-04-26)** | 11-iter prompt-loop run on Gemini 3 Flash, $8.16 of $13.50 budget, 13-case dev suite. v4.8 prompt UNCHANGED in production — every prompt edit attempted (5 distinct mechanisms across iters 6-10) introduced regressions. Real findings: (1) v4.8 is at a brittle local optimum; (2) berries failure unfixable via prompt — needs structural item-type validator (M2 below); (3) off-meta-required failure is `findCounters` calc-spam tool-loop bug — needs consecutive-tool-call cap (M1 below); (4) negative imperatives generalize beyond paragraph scope on Gemini 3 Flash (NEW research finding). Surviving improvements: test predicate fixes, dev set 4→13 cases, paired-comparison harness with anti-cherry-pick gate. Full audit at [runs/prompt-loop/](../runs/prompt-loop/) — HISTORY.md / BEST.md / RATIONALE.md / RESEARCH-INDEX.md (22+ findings) |
| M1 | Consecutive-tool-call cap in route.ts | M | **OPEN — ready to ship (2026-04-26)** | ~30 LOC fix in [src/app/api/team/route.ts](../src/app/api/team/route.ts). Prompt-loop iters 7/8/10 confirmed off-meta-required #1 fails because Gemini 3 Flash spam-calls `calc` 17× after `findCounters` returns, hitting the 20-iter cap. Reproducible 4×. Prompt-only fixes generalize and cause banned-item leaks (iter 9). Fix: after every tool call, if same tool name has fired N≥4 consecutively, inject "you have enough data — emit final blocks now" nudge into `messages` (same pattern as existing `citation_retry` / `team_retry` nudges). Should convert off-meta-required FAIL → PASS deterministically + prevent similar tool-loop bugs on other queries. See [errors.md](errors.md) row "Gemini 3 Flash spam-calls calc" |
| M2 | Pre-flight item-type validator (item-type-guard.ts) | M | **OPEN — ready to ship (2026-04-26)** | ~80 LOC. Build [lib/item-type-guard.ts](../lib/item-type-guard.ts) analogous to [lib/phantom-guard.ts](../lib/phantom-guard.ts). Detect item-type-constraint phrases ("all berries", "different berry per slot", "all type-boost"). Intercept `team_result` event server-side; check each item's `type` field in `items.csv` against the requested type; if mismatch, inject corrective nudge. Prompt-loop iters 6-8 tested 3 prompt approaches (enumeration / priority rule / worked example) and all failed — model's `Mega Pokemon = Mega Stone` heuristic too robust. Prompt-only is empirically exhausted on this case. Wire in [src/app/api/team/route.ts](../src/app/api/team/route.ts) + eval-models.ts (parallel to phantom-guard). See [errors.md](errors.md) row "Gemini 3 Flash item-type-constraint heuristic conflict" |
| M3 | Multi-model bake-off using prompt-loop's 13-case suite | M | **OPTIONAL (2026-04-26 idea)** | Re-run [scripts/prompt-loop/score.ts](../scripts/prompt-loop/score.ts) on `minimax-m2-7`, `minimax-m2-5`, `deepseek-v4-flash`, `kimi-k2-6`, `grok-4-1-fast`, `claude-sonnet-or` to determine if v4.8's brittleness on Gemini 3 Flash is model-specific. ~$1.04/run × 5-6 models = ~$5-6 budget. Yields per-model brittleness/coverage report. Trigger: only after M1+M2 ship (so the same baseline applies to all). Could justify flipping the production default if another model handles the brittle cases more gracefully |
| R-prompt-trim | Token-efficiency loop (relaxed gate variant) | R | **OPTIONAL (2026-04-26 idea)** | Re-run prompt-loop with acceptance gate `paired_score >= 0 AND prompt is >=3% shorter AND no regressions`. Tests whether v4.8 has redundancy that can be cut while preserving 24/26. Per Gemini 3 research, shorter prompts tend to follow instructions better. Trigger: only after M1+M2 (to remove the brittleness floor that currently makes ANY edit regress). Same harness, different gate — minimal new code |
...
| B4 | Speed tier matrix proposal | B | **OPEN — proposal (2026-04-26)** | Pre-compute speed comparisons under speed-control modes (Tailwind, Trick Room, Sand Rush, Swift Swim, Chlorophyll, Surge Surfer, weather pseudo-speed) + expose via a `findSpeedTier(threat, mode?)` agent tool analogous to `findCounters`. Extends existing speed layer: matchup matrix `speed_advantage` col, calc `speedUCurve` / `speedTier` buckets, [data/knowledge/speed_tiers.md](../data/knowledge/speed_tiers.md). See [Part B § B4](#b4--speed-tier-matrix-proposal-not-yet-started) for design notes. Trigger to start: Phase 3 mode-aware behavior settles in prod AND a real-user speed-tier question surfaces that current tools answer poorly |
...
### B4 — Speed tier matrix (proposal, not yet started)

**Status:** PROPOSAL flagged 2026-04-26 by user as future work after Phase 3 (mode-aware system prompt) shipped. Idea: extend the existing speed-handling layer into a queryable speed-tier matrix that the agent can call directly for speed-control planning and analysis-mode questions.

**Current state of the speed layer:**
- [data/knowledge/speed_tiers.md](../data/knowledge/speed_tiers.md) — hand-curated reference doc; indexed in RAG for prose retrieval.
- [lib/calc/matchup.ts](../lib/calc/matchup.ts) — `speedUCurve()` + `speedTier()` bucketed scoring; [matchup_matrix.csv](../matchup_matrix.csv) has a `speed_advantage` column per pair.
- [lib/calc/efficiency.ts](../lib/calc/efficiency.ts) — speed sub-score with Tailwind / Trick Room / priority-net components feeding into composite efficiency.

**Gap.** Players reason about speed in concrete terms — "what speed do I need to outpace Sneasler in Tailwind?", "Mega Excadrill at 103 base outspeeds X in Sand", "what's the slowest I can be and still win Trick Room?". The existing data answers this implicitly but the agent can't query it directly; today it falls back to prose recall against `speed_tiers.md` chunks (and frequently gets numbers wrong).

**Proposed shape.** A `speed_matrix.csv` pre-computed for all 245 sets (186 base + 59 Mega forms) under multiple speed-control modes — base, Tailwind ×2, Trick Room reverse-order, Sand Rush ×2 in sand, Swift Swim ×2 in rain, Chlorophyll ×2 in sun, Surge Surfer ×2 on Electric Terrain, +1 Speed setup (Dragon Dance / Agility / Rock Polish / Quick Feet status). Plus a `findSpeedTier(threat, mode?, mySpeed?)` agent tool that returns: "X outspeeds threat by N points in mode M", "X needs +Y SP to outspeed threat", "lowest speed that still outpaces threat in mode M". Tool surface mirrors `findCounters` — calc-derived data, NO chunk_ids.

**Why it matters.**
- **Team-build mode**: speed-control planning becomes calc-grounded ("Tyranitar at 71 base needs Tailwind support to outpace Sneasler in mirror; Mega Aerodactyl at 150 base outspeeds the field naturally") instead of guess-and-search.
- **Analysis mode**: handles "speed creep" questions and "what's the optimal Speed SP for X under Tailwind to outpace common Y / Z threats" directly with structured numbers.

**Open design questions.**
- Effective speed = base × nature × (1 + SP/64) × ability/item/mode multiplier — should the matrix encode max-investment effective speed, or a small grid of common SP allocations (0/16/32 in Speed)?
- Per-mode pre-compute or compute on demand at query time? Pre-compute keeps the latency tiny and matches the `findCounters` pattern; cost is ~245 × 9 modes ≈ 2.2k rows.
- Stacking modifiers (Tailwind + Sand Rush + Choice Scarf) — which combinations are worth pre-computing? Likely just the common archetype combinations (Tailwind + Sand Rush, TR + weather sweepers).
- Sticky Web / Icy Wind / Electroweb stack effects — probably out of scope for v1; the static matrix can answer "after −1 Speed" with a single subtraction.
- Is there value in a structured `data/computed/speed_tiers.json` that both the RAG indexer and the new tool consume? Avoids drift between the matrix and the markdown doc.
- Does the agent need the tool, or is enriching `speed_tiers.md` and improving its retrieval boost enough? Tool wins on calc precision; doc wins on cross-cutting speed-control strategy that's not just numbers.

**Trigger to start.** When (1) Phase 3 mode-aware behavior settles in real-user usage AND (2) a concrete speed-tier question surfaces that the current `calc` + `findCounters` + `search`-against-`speed_tiers.md` path answers poorly. Until both, defer.

**Pointer.** This memo is the design seed; expand into a phased plan when the trigger fires.
